...
public class CheckAndUpdateProcedure : ProcedureBase
{
    ...
    private void OnCheckResurcesComplete(int movedCount, int removedCount, int updateCount, long updateTotalLength, long updateTotalZipLength)
    {
        ...
        {
            Log.Info("需要更新资源个数:{0},资源大小:{1},下载地址:{2}", updateCount, updateTotalZipLength, GFBuiltin.Resource.UpdatePrefixUri);
    ...
    private void OnResourceUpdateFailure(object sender, GameEventArgs e)
    {
        ...
        {
            Log.Error("Download '{0}' failure from '{1}' with error message '{2}', retry count '{3}'.", ne.Name, ne.DownloadUri, ne.ErrorMessage, ne.RetryCount.ToString());
            return;
        }
        else
        {
            Log.Info("Download '{0}' failure from '{1}' with error message '{2}', retry count '{3}'.", ne.Name, ne.DownloadUri, ne.ErrorMessage, ne.RetryCount.ToString());
        }
